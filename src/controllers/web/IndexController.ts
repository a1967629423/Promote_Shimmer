import {Controller,Get,Render, Req, Res, UseBefore, BodyParam} from 'routing-controllers';
import { UserInfo } from '../../services/UserCenterService';
import {ConfigService} from '../../services/ConfigService';
import Container from 'typedi'
import qrcode from 'qrcode'
import { GetUserMiddleware } from '../../middlewares/local/GetUserMiddleware';
import { SessionMiddleware } from '../../middlewares/local/SessionMiddleware';
@Controller()
export class PageController {
    @Get('/')
    @Render("index")
    async hello() {
        return {}
    }

    @Get('/form')
    @Render('form')
    @UseBefore(SessionMiddleware,GetUserMiddleware)
    async formPage(@Req() request:any,@Res() resolve:any,@BodyParam('userInfo') userInfo:UserInfo) {
        console.log(userInfo);
        return {userInfo};
        
    }

    @Get('/poster')
    @Render('poster')
    @UseBefore(SessionMiddleware,GetUserMiddleware)
    async posterPage(@BodyParam('userInfo') userInfo:UserInfo) {
        let fullurl = Container.get(ConfigService).config.webSideFullUrl;
        let qr={url:''}
        if(userInfo.is_login)
        {
            qr.url = await qrcode.toDataURL(`${fullurl}/form?vendor=${userInfo.openid}`,{type:'image/jpeg',margin:0,width:200})
            return {userInfo,qr}
        }
        return {userInfo}
    }

    @Get('/userCenter')
    @Render('userCenter')
    @UseBefore(SessionMiddleware,GetUserMiddleware)
    async userCenterPage(@BodyParam('userInfo') userInfo:UserInfo) {
        return {userInfo}
    }
}